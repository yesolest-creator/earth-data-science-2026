...
/* navigator.clipboard가 없거나(파일을 직접 열었을 때 등 보안 컨텍스트가 아닌 경우) 거부되면
   숨겨진 textarea + execCommand("copy")로 대체한다. 모든 복사 버튼이 이 함수를 공유한다. */
function step3CopyFallback(text) {
  return new Promise(function (resolve, reject) {
    try {
      ...
      if (ok) resolve(); else reject(new Error("execCommand 실패"));
    } catch (err) {
      reject(err);
    }
  });
...
function initCopyButtons() {
  document.querySelectorAll(".codeblock .copy").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      if (!pre) return;
      copyTextToClipboard(pre.innerText).then(function () {
...
function step3RenderExampleChart() {
  ...
  step3RenderChartNote(keys, byClass);
}

/* 04. 그래프 아래 한 줄 정리 — 학생이 만들 웹앱에도 이런 문장이 붙으면 좋다는 본보기 */
function step3RenderChartNote(keys, byClass) {
  const note = document.getElementById("example-chart-note");
  if (!note) return;

  let hi = null, lo = null;   // hi = E 비율 최고, lo = E 비율 최저(= I 비율 최고)
  keys.forEach(function (k) {
    const c = byClass[k];
    if (!c.total) return;
    const p = c.a / c.total;
    if (!hi || p > hi.p) hi = { k: k, p: p, c: c };
    if (!lo || p < lo.p) lo = { k: k, p: p, c: c };
  });

  if (!hi || !lo || hi.k === lo.k) {
    note.innerHTML = "여덟 개 반의 E/I 비율이 거의 비슷하게 나왔습니다.";
    return;
  }

  const hiPct = Math.round(hi.p * 100);
  const loPct = Math.round((1 - lo.p) * 100);
  const gap = hiPct - Math.round(lo.p * 100);

  note.innerHTML =
    'E가 가장 많은 반은 <strong>' + step3EscapeHtml(hi.k) + '반</strong>(E ' + hiPct + '%), ' +
    'I가 가장 많은 반은 <strong>' + step3EscapeHtml(lo.k) + '반</strong>(I ' + loPct + '%)입니다. ' +
    '같은 학년인데도 두 반의 E 비율 차이가 ' + gap + '%p나 되네요. ' +
    '한 반이 스무 명 남짓이라 몇 명 차이로도 비율이 크게 흔들린다는 점은 같이 생각해볼 만합니다.';
}
...
function step3AttachCopy(btn, getText, okMsg) {
  ...
  btn.addEventListener("click", function () {
    ...
    copyTextToClipboard(text).then(function () {
      ...
      if (status) { status.textContent = "복사에 실패했습니다. 아래 '공통 규칙 전체 보기'를 열어 직접 선택해 복사해주세요."; status.className = "lstate err"; }
    });
  });
}
...
function step3InitBuilder() {
  ...
  btn.addEventListener("click", function () {
    ...
    copyTextToClipboard(text).then(function () {
      ...
      if (status) { status.textContent = "복사에 실패했습니다. 위 '전체 규칙 보기'를 열어 직접 선택해 복사해주세요."; status.className = "lstate err"; }
    });
  });
}
